...
RSpec.shared_examples "rest_cmds_controller" do |klass, klass_presenter|
  describe "GET #index" do                                                             
    it "calls Cmds::Query with a Criteria" do
      expect(Cmds::Query).to receive(:new).with(klass,  kind_of(Criteria)).and_return(double("command", call: 'prez') )
      get :index, {}, valid_session
      expect(assigns(:prez)).to eq('prez')
    end
  end

  describe "GET #show" do
    it "calls Cmds::Find with a id" do
      expect(Cmds::Find).to receive(:new).with(klass, object.id.to_s).and_return(double("command", call: 'prez') )
  ...
  describe "POST #create" do
    context "calls Cmds::Create" do
      ...
      it "ok" do
        expect_any_instance_of(Cmds::Create).to receive(:call).and_return(double(target: mock_object))
    ...
    context "with invalid params" do
      it "re-renders the 'new' template" do
        expect_any_instance_of(Cmds::Create).to receive(:call).and_return(double(target: klass.new))
  ...
  describe "PUT #update" do
    context "with valid params" do
      it "redirects to the object show page" do
        expect_any_instance_of(Cmds::Update).to receive(:call).and_return(double(target: object, errors: nil))
    ...
    context "with invalid params" do
      it "assigns the object as @object" do
        expect_any_instance_of(Cmds::Update).to receive(:call).and_return(double(target: object, errors: "err message"))
  ...
  describe "DELETE #destroy" do
    ...
    it "redirects to the objects list" do
      expect_any_instance_of(Cmds::Destroy).to receive(:call)
