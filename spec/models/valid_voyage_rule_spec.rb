...
RSpec.describe ValidVoyageRule, type: :model do
  ...
  context "not _satisfied?  voyage no movements" do 
    When(:result)  { ValidVoyageRule.new(voyage).satisfied?(recital) }
    Then           { ! result } 
    Then           { recital.explain == 'Schedule is Empty' }
  end 
  ...
  context "one movement" do
    ...
    context "not _satisfied?  voyage open loop" do 
      When(:result)  { ValidVoyageRule.new(voyage).satisfied?(recital) }
      Then           { ! result }
      Then           { recital.explain == 'Schedule does not end in home port' }
    end 
    ...
    context "_satisfied?  voyage 2-hops complete loop" do 
      ...
      When(:result)  { ValidVoyageRule.new(voyage).satisfied?(recital) }
      Then           { result }
      Then           { recital.explain == 'Schedule is complete'}
    end 
  end
